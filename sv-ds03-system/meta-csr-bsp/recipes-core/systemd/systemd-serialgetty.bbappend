...
ENABLE_AUTOLOGIN = "y"
ENABLE_AUTOLOGIN_atlas7-product-mode = "n"

do_install() {
	if [ ! -z "${SERIAL_CONSOLES}" ] ; then
		if [ ${ENABLE_AUTOLOGIN} == "n" ]; then
			sed -i -e '/\[Unit\]/a\BindsTo=dev-%i.device' ${WORKDIR}/serial-getty@.service
			sed -i 's/--autologin root //' ${WORKDIR}/serial-getty@.service
		fi
