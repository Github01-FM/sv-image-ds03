...
status_t AudioHardware::setMicMute(bool state)
{
    ...
    sp<AudioStreamInALSA> spIn;
    {
		...
		int mic_volume_max, mic_volume_min;
		unsigned int volume_num, i;
		struct mixer_ctl *mic_volume_ctl;
		openMixer_l();
		mic_volume_ctl = mixer_get_ctl_by_name(mMixer, "Capture Volume");
		if (mic_volume_ctl) {
...
{
    struct mixer_ctl *headphone_ctl = NULL;
    ...
    if (mMixer) {
        ...
        TRACE_DRIVER_IN(DRV_MIXER_GET)
        mRouteCtl = mixer_get_ctl_by_name(mMixer, "Speaker Switch");
	headphone_ctl = mixer_get_ctl_by_name(mMixer, "Headphone Switch");
        TRACE_DRIVER_OUT
    }
    LOGV("write() wakeup setting route %s", route);
    if (mRouteCtl && headphone_ctl) {
	unsigned int num_values;
	unsigned int hp_num_values;
	unsigned int i;
        num_values = mixer_ctl_get_num_values(mRouteCtl);
	hp_num_values = mixer_ctl_get_num_values(headphone_ctl);
        TRACE_DRIVER_IN(DRV_MIXER_SEL)
	for (i = 0; i < num_values; i++) {
	    if (mDevices & (AudioSystem::DEVICE_OUT_SPEAKER | AudioSystem::DEVICE_OUT_EARPIECE))
	        mixer_ctl_set_value(mRouteCtl, i, 1);
	    else
	        mixer_ctl_set_value(mRouteCtl, i, 0);
	}
	for (i = 0; i < hp_num_values; i++) {
	    if (mDevices & AudioSystem::DEVICE_OUT_WIRED_HEADSET)
		mixer_ctl_set_value(headphone_ctl, i, 1);
	    else
		mixer_ctl_set_value(headphone_ctl, i, 0);
	}
        TRACE_DRIVER_OUT
    }
    return NO_ERROR;
